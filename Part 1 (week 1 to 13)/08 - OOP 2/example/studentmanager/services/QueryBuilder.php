...
class QueryBuilder
{
    ...
    public function whereIn(array $keyValue): QueryBuilder
    {
        $this->whereIn = $keyValue;
        return $this;
    }
    ...
    public function get(): array
    {
        ...
        if ($this->whereIn) {
            ...
            $sql .= key($this->whereIn) . " IN (" . implode(', ', array_map(fn() => '?', $this->whereIn)) . ")";
        }
        return $this->pdoService->fetch($sql, array_values(array_merge($this->where, $this->whereIn)), $this->className);
    }
